(function exampleCode() {
	...
	function loadModels() {
		...
		if (t3d) {
			...
			t3d.removeAll();
			t3d.loadModel("assets/brfv4_occlusion_head.json", numFacesToTrack);
		}
	}
	...
	brfv4Example.updateCurrentExample = function (brfManager, imageData, draw) {
		...
		for (var i = 0; i < faces.length; i++) {
			...
			if (face.state === brfv4.BRFState.FACE_TRACKING) {
				...
				draw.drawTriangles(face.candideVertices, face.candideTriangles, false, 1.0, 0xffda84, 0.4);
				draw.drawVertices(face.candideVertices, 2.0, false, 0xffda84, 0.4);
